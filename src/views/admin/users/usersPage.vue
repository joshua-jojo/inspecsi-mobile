<template lang="">
    <div
        class="relative justify-between items-center h-[6rem] bg-primary text-primary-content">
        <div
    ...
    <div class="px-4">
        <div class="flex justify-between gap-2 my-4">
            <input
                type="text"
                placeholder="Search"
                class="input text-[20px] font-poppins bg-[#E9F8F9] input-bordered w-full border-[2.5px] border-primary rounded-xl" />
        ...
        <div class="grid place-items-center gap-3 mb-5">
            <div class="card w-full text-white text-xl shadow mb-7">
                <label
                    for="tambah"
                    class="btn btn-primary capitalize font-poppins-semibold">
                    buat user baru
                </label>
            </div>
            <template v-for="item in dataAllUser" :key="item.id">
                <div class="card bg-[#E9F8F9] w-full text-primary shadow">
                    <div
                        ...
                        <div
                            class="flex justify-center items-center h-full w-full max-w-[50px] overflow-hidden drop-shadow-md">
                            <img
                                :src="item.foto"
                                ...
                                srcset=""
                                v-if="item.foto" />
                            <div class="avatar placeholder w-32" v-else>
                                <div
                                    class="bg-base-300 text-neutral-content rounded-full w-full h-full">
                                    <span class="text-2xl">{{
                                        item.name.slice(0, 1)
                                    }}</span>
                                </div>
        ...
        <modalComponent id="tambah" title="tambah user">
            <div class="form-control font-poppins-semibold">
                ...
                <input
                    ...
                    v-model="buatUser.name" />
                <label class="label" v-if="buatUser.errors.name"
                    ><span class="label-text-alt text-error">{{
                        buatUser.errors.name
                    }}</span></label
                >
            </div>
            <div class="form-control font-poppins-semibold">
                ...
                <input
                    ...
                    v-model="buatUser.email" />
                <label class="label" v-if="buatUser.errors.email"
                    ><span class="label-text-alt text-error">{{
                        buatUser.errors.email
                    }}</span></label
                >
            </div>
            <div class="form-control font-poppins-semibold">
                ...
                <select
                    class="input input-bordered capitalize"
                    v-model="buatUser.role_id">
                    <option :value="null" disabled>Pilih role</option>
                    <template v-for="item in dataRole" :key="item.id">
                        <option :value="item.id">{{ item.nama }}</option>
                    </template>
                </select>
                <label class="label" v-if="buatUser.errors.role_id"
                    ><span class="label-text-alt text-error">{{
                        buatUser.errors.role_id
                    }}</span></label
                >
            </div>
            <div class="form-control font-poppins-semibold">
                ...
                <input
                    ...
                    v-model="buatUser.password" />
                <label class="label" v-if="buatUser.errors.password"
                    ><span class="label-text-alt text-error">{{
                        buatUser.errors.password
                    }}</span></label
                >
            </div>
            <div class="form-control font-poppins-semibold">
                ...
                <input
                    ...
                    v-model="buatUser.password_confirmation" />
            </div>
            <template v-slot:action>
                <button class="btn btn-primary" @click="sendNewUser()">
                    simpan
                </button>
            </template>
        </modalComponent>
        ...
        <modalComponent :title="`Hapus user ${hapusUser.nama}`" id="hapus">
            <template v-slot:action>
                <button
                    @click="submitHapus"
                    :class="{ 'loading btn-disabled': hapusUser.processing }"
                    class="btn btn-error">
                    konfirmasi
                </button>
            </template>
